...
private:
  ...
  public:
    ...
    Model<MODELNAME>(parser,runPar.getLatParams(), toolBox, runPar.dt, STRINGIFY(MODELLABEL)) //MODELLABEL is defined in the cmake.
    {
        ...
        Mphi = M/phii;
        Lphi = Lambda/phii;
        omega = phii*pow<2>(Lphi)/pow<pExp/2>(Mphi);//sqrt(Lambda4)/M;
        g = sqrt(q)*omega/phii;
    ...
    auto potentialTerms(Tag<0>){ return pow<pExp>(Mphi) * pow<pExp>(tanh(fldS(0_c)/Mphi)) / pExp; }
    auto potentialTerms(Tag<1>){ return 0.5 * q * pow<2>(fldS(0_c)) * pow<2>(fldS(1_c)); }
    
    auto potDeriv(Tag<0>){ return 2*pow<(pExp-1)>(M/phii) * pow<pExp>(tanh(fldS(0_c)/Mphi))/sinh(2*fldS(0_c)/Mphi) + q * fldS(0_c) * pow<2>(fldS(1_c)); }
    auto potDeriv(Tag<1>){ return  q * fldS(1_c) * pow<2>(fldS(0_c)); }

    auto potDeriv2(Tag<0>){ return  4 * pow<(pExp-2)>(Mphi) * (pExp - cosh(2*fldS(0_c)/Mphi) ) * pow<pExp>(tanh(fldS(0_c)/Mphi)) / pow<2>(sinh(2*fldS(0_c)/Mphi)) +  q * pow<2>(fldS(1_c)); }
